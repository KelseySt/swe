...
export default function CompanyClientPage({ company, metrics }: { company: string; metrics: Metric[] }) {
  ...
  return (
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[1200px] p-10 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="p-6 border rounded-xl shadow-lg w-full col-span-1">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Net Margin Trend</h3>
    ...
    <div className="text-red-500 mt-2">Low Profit Margin: Below 5%</div>
  )}
  {currentData.netMargin >= 5 && currentData.netMargin < 10 && (
    <div className="text-orange-500 mt-2">Moderate Profit Margin: 5% - 10%</div>
  )}
  {currentData.netMargin >= 10 && (
    <div className="text-green-500 mt-2">Healthy Profit Margin: Above 10%</div>
  )}
            
          </div>
          <div className="p-6 border rounded-xl shadow-lg w-full col-span-1">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Net Margin Comparison</h3>
            <div className="flex flex-col space-y-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className={`text-lg font-semibold ${isIncrease ? 'text-green-500' : 'text-red-500'}`}>
                  {Math.abs(netMarginChange).toFixed(2)}% ({isIncrease ? '↑' : '↓'} {percentageChange}%)
                </span>
              </div>
            </div>
          </div>
          <div className="p-6 border rounded-xl shadow-lg w-full col-span-1">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Operating Margin</h3>
            <div className="text-2xl font-semibold text-gray-700 mb-4">{currentData.operatingMargin}</div>
            <LineChart width={300} height={150} data={metrics}>
              <Line type="monotone" dataKey="operatingMargin" stroke="#34d399" />
              <XAxis dataKey="fiscalYear" />
              <YAxis />
              <Tooltip />
              <Legend />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="p-6 border rounded-xl shadow-lg w-full">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Current Ratio</h3>
            <div className="text-2xl font-semibold text-gray-700 mb-4">{currentData.currentRatio}</div>
            <LineChart width={300} height={150} data={metrics}>
    ...
    <div className="text-red-500 mt-2">Critical: Current Ratio is below 1.</div>
  )}
  {currentData.currentRatio >= 1 && currentData.currentRatio < 1.2 && (
    <div className="text-orange-500 mt-2">Warning: Current Ratio is below 1.2.</div>
  )}
  {currentData.currentRatio >= 1.2 && currentData.currentRatio <= 2 && (
    <div className="text-green-500 mt-2">Healthy: Current Ratio is within range (1.2 - 2).</div>
  )}
          </div>

<div className="p-6 border rounded-xl shadow-lg w-full">
  <h3 className="text-xl font-semibold text-gray-800 mb-4">Quick Ratio</h3>
  <div className="text-2xl font-semibold text-gray-700 mb-4">{currentData.quickRatio}</div>
  <LineChart width={300} height={150} data={metrics}>
  ...
  {currentData.quickRatio < 1 && (
    <div className="mt-4 flex items-center text-red-500">
      ...
      <span className="text-sm font-medium">Warning: Quick Ratio is below 1!</span>
    </div>
  )}
</div>
          <div className="p-6 border rounded-xl shadow-lg w-full">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Debt to Equity</h3>
            <LineChart width={300} height={150} data={metrics}>
